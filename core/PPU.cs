...
public class PPU
{
    ...
    private int _countCycles = 0;
    public PPU(ref MMU mmu, ref DrawingArea drawingArea)
    {
        this._mmu = mmu;
    ...
    */
    public void Update(int cycles)
    {
        ...
        // LCD 是否啟用 (LCDC bit 7 為 1 表 LCD 啟用)
        if (((_mmu.LCDC >> 7) & 1) == 1)
        {
            /* 進行 PPU 更新*/
    ...
    /*
        Tile 是 8x8 像素的圖塊，每個圖塊需要 16 字節的存儲空間。
        Tile Map 定義了背景的結構，記錄每個圖塊在畫面中的位置。
        Tile Data 是圖塊的實際像素數據，用於決定每個像素的顏色。
        BG : 256 x 256 --> 每行有 32 個 Tile

        - 背景偏移量：SCX(水平), SCY(垂直)
        - Tile Map 地址範圍由 LCDC (Bit 3)決定：
            0:0x9800 ~ 0x9BFF
            1:0x9C00 ~ 0x9FFF
        - Tile Data 地址範圍由 LCDC (Bit 4)決定：
            0:0x8800 ~ 0x97FF
            1:0x8000 ~ 0x8FFF
    ...
    */
    private void BGToBuffer()
    {
        ...
        for (int x = 0 ; x < Global.SCREEN_WIDTH ; x ++)
        {
            ...
            int signedTileID = (tileID >= 128) ? tileID - 256 : tileID;
            u16 tileDataAddress = (u16)(tileDataBase + signedTileID * 16);
    ...
    private void SpritesToBuffer() 
    {
        for (int i = 0x9C ; i >= 0 ; i -= 4) 
        {
            ...
            if ((_mmu.LY >= y) && (_mmu.LY < y + ((((_mmu.LCDC >> 2) & 1) == 1) ? 16 : 8))) 
            {
                ...
                u8 low = _mmu.VRAM[tileAddress];
                u8 high = _mmu.VRAM[tileAddress + 1];
