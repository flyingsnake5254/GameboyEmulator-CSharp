public class Instructions
{
    ...
    private void LD(IncsType incsType, string data1 = "", string data2 = "")
    {
        if (incsType == IncsType.R16_D16)
        {
            switch (data1)
            {
                case "BC": _regs.BC = _mmu.Read16(_regs.PC); break;
                case "DE": _regs.DE = _mmu.Read16(_regs.PC); break;
                case "HL": _regs.HL = _mmu.Read16(_regs.PC); break;
                case "SP": _regs.SP = _mmu.Read16(_regs.PC); break;
            }
            _regs.PC += 2; 
            _cycles += 12;
        }
        ...
        {
            _mmu.Write16(_mmu.Read16(_regs.PC), _regs.SP);
            _regs.PC += 2;
            _cycles += 20;
        }
        ...
        {
            _mmu.Write(_mmu.Read16(_regs.PC), _regs.A);
            _regs.PC += 2;
            _cycles += 16;
        }
        else if (incsType == IncsType.R8_A16)
        {
            _regs.A = _mmu.Read(_mmu.Read16(_regs.PC));
            _regs.PC += 2;
            _cycles += 16;
        }
    }
    ...
    private void HALT()
    {
        if (!_ime && ((_mmu.IERegister & _mmu.IFRegister & 0x1F) == 0)) { _isHalted = true; _regs.PC --;}
        _cycles += 4;
    }
    ...
    {
        u16 value = _mmu.Read16(_regs.SP);
        _regs.SP += 2;
    ...
    private void PUSH(string data1, IncsType incsType = IncsType.R16, u16 number = 0)
    {
        if (incsType == IncsType.R16)
        {
            ...
            _regs.SP -= 2;
            _mmu.Write16(_regs.SP, value);
            _cycles += 16;
        }
        else if (incsType == IncsType.A16)
        {
            _regs.SP -= 2;
            _mmu.Write16(_regs.SP, number);
            _cycles += 16;
        }
    ...
    private void JP(bool flag, IncsType incsType = IncsType.A16)
    {
        ...
        {
            if (flag) { _regs.PC = _mmu.Read16(_regs.PC); _cycles += 16; }
